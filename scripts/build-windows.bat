...
:build_project
call :print_status "Building simple-nfsd..."
...
:install_project
if "%INSTALL%"=="true" (
    call :print_status "Installing simple-nfsd..."
    cd /d "%BUILD_DIR%"
...
if "%ENABLE_STATIC_LINKING%"=="ON" (
    ...
    set "STATIC_DIR=%DIST_DIR%\simple-nfsd-0.1.0-static-windows"
    if not exist "%STATIC_DIR%" mkdir "%STATIC_DIR%"
    
    REM Copy binary and files
    copy "%BUILD_DIR%\simple-nfsd.exe" "%STATIC_DIR%\"
    ...
    cd /d "%DIST_DIR%"
    powershell -Command "Compress-Archive -Path 'simple-nfsd-0.1.0-static-windows' -DestinationPath 'simple-nfsd-0.1.0-static-windows.zip' -Force"
    rmdir /s /q "simple-nfsd-0.1.0-static-windows"
    
    call :print_success "Static binary package created: simple-nfsd-0.1.0-static-windows.zip"
)
goto :eof

REM Function to show usage
:show_usage
echo simple-nfsd - Windows Build Script
echo.
...
REM Main execution
:main
call :print_status "Starting simple-nfsd build process..."
...
call :print_success "simple-nfsd build process completed successfully!"
goto :end
